...
CURLcode Curl_fillreadbuffer(struct connectdata *conn, int bytes, int *nreadp)
{
  ...
  if(conn->handler->protocol&(CURLPROTO_SPDY)) {
    //Do spdy specific here
    buffersize -= (8);   /* 8 bytes data frame */
    data->req.upload_fromhere += (8); /* 8bytes data frame */
  }
  else
#endif
  ...
  if(conn->handler->protocol&(CURLPROTO_SPDY)) {
    ...
    fprintf(stderr, "%s:%d strem %p\n",
      __FUNCTION__, __LINE__, data->req.stream_client);
    spint = spindly_stream_data(data->req.stream_client, flags,
    data->req.upload_fromhere, nread, NULL);

    data->req.upload_fromhere -= 8;
    spint = spindly_phys_outgoing(conn->phys_client,
      &spdy_data, &len);

    if(len)
      memcpy (data->req.upload_fromhere, spdy_data, 8);
    spint = spindly_phys_sent(conn->phys_client, len);
