...
static const npn_proto proto_list[] = {
  { (const unsigned char*)"spdy/3", 6, 3 },
  { (const unsigned char*)"spdy/2", 6, 2 }
};
                                 ...
                                 const unsigned char *in, unsigned int inlen)
{
  ...
  for(; i < inlen; i += in[i]+1) {
    unsigned int j;
    for(j = 0; j < sizeof(proto_list)/sizeof(npn_proto); ++j) {
                                ...
                                void *arg)
{
  ...
  /* select_next_protocol() selects SPDY protocol version the
   Spdylay library supports. */
  rv = select_next_protocol(out, outlen, in, inlen);
  //if(rv <= 0) {
  //  fprintf(stderr, "[venki] did not spdy/2 or spdy/3 protocol.");
  //}
  //else
  //  fprintf(stderr, "[venki]%s:%d spdy version %d \n",
                   ...
                   int sockindex)
{
  ...
  */
  ...
  if(data->set.ssl.version == CURL_SSLVERSION_DEFAULT) {
    /*
     * Tizen patch
     * TLSv1.1/1.2 disabled for backward compatibility with old SSL server
     */
    ctx_options |= SSL_OP_NO_SSLv2 | SSL_OP_NO_TLSv1_1 | SSL_OP_NO_TLSv1_2;
  }
  ...
  if(data->set.allowed_protocols & CURLPROTO_SPDY) {
    //fprintf(stderr, "[venki]%s:%d proto_select_cb\n",
    //__FUNCTION__, __LINE__);
    uint16_t spdy_proto_version;
                           ...
                           bool strict)
{
  ...
  if(data->set.ssl.certinfo) {
    ...
    retcode = get_cert_chain(conn, connssl);
    if(retcode) {
      failf(data, "SSL: couldn't get cert chain!");
      return retcode;
    }
  }
