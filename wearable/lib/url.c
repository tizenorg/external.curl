...
CURLcode Curl_init_userdefined(struct UserDefined *set)
{
  ...
  set->allowed_protocols = CURLPROTO_ALL & ~CURLPROTO_SPDY;
  set->redir_protocols =
...
#ifdef USE_SPDY
  if(conn->phys_client) {
    spindly_phys_cleanup(conn->phys_client);
    conn->phys_client = NULL;
  }
#endif
  if(!data) {
                                 ...
                                 const struct connectdata *conn)
{
  ...
  if(conn->handler->protocol & CURLPROTO_SPDY) {
    fprintf(stderr, "%s:%d spdy support\n", __FUNCTION__, __LINE__);
    return TRUE;
  }
#endif
                 ...
                 struct connectdata **usethis)
{
  ...
  //fprintf(stderr, "%s:%d  connects %p\n", __FUNCTION__, __LINE__,
    //data->state.connc->connects);
...
#ifdef USE_SPDY
            && !(needle->handler->protocol & CURLPROTO_SPDY)
            && !(check->handler->protocol & CURLPROTO_SPDY)
#endif
            ) {
            DEBUGF(infof(data,
                             ...
                             const char *protostr)
{
  ...
  //Need to check other handlers also, whether scheme supported
...
#define LEAST_PATH_ALLOC 256
  ...
  if(proxy) {
    ...
    if((conn->proxytype == CURLPROXY_HTTP) ||
      ...
      if(!(conn->handler->protocol & (CURLPROTO_HTTP|CURLPROTO_SPDY)) &&
