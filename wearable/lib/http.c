...
#ifdef USE_SPDY
/*
 * HTTP spdy handler interface.
 */
const struct Curl_handler Curl_handler_spdy_http = {
  "HTTP",                              /* scheme */
  ZERO_NULL,                            /* setup_connection */
  Curl_spdy,                            /* do_it */
  Curl_spdy_done,                       /* done */
  ...
  ZERO_NULL,                            /* perform_getsock */
  ZERO_NULL,                            /* disconnect */
  Curl_spdy_readwrite,                            /* readwrite */
  PORT_HTTPS,                           /* defport */
  CURLPROTO_SPDY,     /* protocol */
  PROTOPT_SSL                           /* flags */
};


/*
 * HTTPS spdy handler interface.
 */
const struct Curl_handler Curl_handler_spdy = {
  "HTTPS",                              /* scheme */
  ZERO_NULL,                            /* setup_connection */
  Curl_spdy,                            /* do_it */
  Curl_spdy_done,                       /* done */
...
#ifdef USE_SPDY
  if(data->set.allowed_protocols & CURLPROTO_SPDY) {
    if(conn->phys_client == NULL)
      conn->phys_client = spindly_phys_init(SPINDLY_SIDE_CLIENT,
...
#ifdef USE_SPDY
/*
 * Curl_spdy_done() gets called from Curl_done() after a single HTTP request
 * has been performed.
 */

CURLcode Curl_spdy_done(struct connectdata *conn,
                        CURLcode status, bool premature)
{
  struct SessionHandle *data = conn->data;
  struct HTTP *http =data->state.proto.http;
  ...
  Curl_unencode_cleanup(conn);
  //fprintf(stderr, "[venki]%s:%d enter\n", __FUNCTION__, __LINE__);
  ...
  //TODO: if any checks required before return
  return CURLE_OK;
}

/* check and possibly add an Expect: header */
static CURLcode spdyexpect100(struct SessionHandle *data,
                          struct connectdata *conn,
                          struct spindly_headers *header)
{
  CURLcode result = CURLE_OK;
  const char *ptr;
  data->state.expect100header = FALSE; /* default to false unless it is set
  ...
  if(use_http_1_1(data, conn)) {
    ...
    if(ptr) {
      data->state.expect100header =
        ...
        CURLcode result = spindly_add_header(header,
          "Expect", "100-continue");
        if(result)
            return result;
        data->state.expect100header = TRUE;
...
CURLcode Curl_spdy_add_custom_headers(struct connectdata *conn,
                                   struct spindly_headers *header)
{
  char *ptr;
  struct curl_slist *headers=conn->data->set.headers;
      ...
      if(*ptr) {
        ...
        if(conn->allocptr.host &&
          ...
          ;
                ...
                checkprefix("Connection", headers->data))
          ;
        else if(/* don't pass on a custom Connection: */
                checkprefix("Connection", headers->data))
          ;
        else {
          pch = strtok (headers->data,": ");
          pch2 = strtok (NULL,"");
            if(pch2[0]==' ')
              pch2 = pch2+1;

          CURLcode result = spindly_add_header(header, pch, pch2);
          if(result)
            return result;
        }
      }
        ...
        if(*ptr) {
          ...
          if(*(--ptr) == ';') {
            ...
            *ptr = ':';
            ...
            result = spindly_add_header(header, pch, pch2);
            if(result)
              return result;
          }
        }
...
CURLcode Curl_spdy_add_timecondition(struct SessionHandle *data,
                                struct spindly_headers *header)
{
  const struct tm *tm;
  char *buf = data->state.buffer;
  ...
  case CURL_TIMECOND_IFMODSINCE:
  default:
    result = spindly_add_header(header,"If-Modified-Since", buf);
    break;
  case CURL_TIMECOND_IFUNMODSINCE:
    result = spindly_add_header(header,"If-Unmodified-Since", buf);
    break;
  case CURL_TIMECOND_LASTMOD:
    result = spindly_add_header(header,"Last-Modified", buf);
    break;
  }
...
CURLcode Curl_spdy_readwrite(struct SessionHandle *data,
                struct connectdata *conn, ssize_t *nread, bool *readmore)
{
  ...
  int read = *nread;
  char* buffer=k->buf;
  if(*nread>0 && conn->phys_client) {
    struct spindly_demux demux;
    spindly_error_t spint;
    spint = spindly_phys_incoming(conn->phys_client,
                                                    (unsigned char *)k->buf,
                                                    *nread,
    ...
    *nread = 0;*/
    ...
    do {
      /* demux the incoming data to figure out if there's anything to do*/
      memset(&demux, 0, sizeof(struct spindly_demux));
      spint = spindly_phys_demux(conn->phys_client, &demux);
      if((spint != SPINDLYE_OK) &&
        (spint != SPINDLYE_DATA_LEFT)) {
        fprintf(stderr, "return here %d", spint);
        return -1;
       }
      //fprintf(stderr, "[venki]%s:%d demux %d \n", __FUNCTION__, __LINE__,
      // demux.type);
      switch(demux.type) {
      case SPINDLY_DX_STREAM_REQ:
            //fprintf(stderr, "[venki]%s:%d streamid %d \n", __FUNCTION__,
            //  __LINE__, demux.msg.stream.streamid);
        break;
      case SPINDLY_DX_STREAM_ACK: {
          ...
          int i=0;
          size_t len;
              //fprintf(stderr, "[venki]%s:%d syn_reply streamid %d \n",
              //  __FUNCTION__, __LINE__, demux.msg.stream_ack.streamid);
          header = &(demux.msg.stream_ack.headers);
          pairs = header->pairs;
          for(i = 0 ; i < header->count; i++) {
            if(strcmp(pairs[i].name, ":status")==0) {
              sscanf(pairs[i].value, "%3d", &k->httpcode);
              data->info.httpcode = k->httpcode;
              //fprintf(stderr, "[venki]%s:%d :%d \n",
              //  __FUNCTION__, __LINE__, k->httpcode);
          }
            ...
            data->req.headerbytecount += (long)len;
              ...
              result = Curl_client_write(conn, writetype, pheader, len);
            Curl_safefree(pheader);

              if(result) {
                spindly_free_demux(conn->phys_client, &demux);
                return result;
              }
          }
        }
      ...
      case SPINDLY_DX_DATA: {
              ...
              if(data->set.verbose)
                Curl_debug(data, CURLINFO_HEADER_IN,
                           demux.msg.data.datap, demux.msg.data.len, conn);
              if(demux.msg.data.datap)
                len = strlen(demux.msg.data.datap);
              //fprintf(stderr, "[venki]%s:%d %d :%d \n",
              //  __FUNCTION__, __LINE__, len, demux.msg.data.len);

              if(demux.msg.data.len) {
                result = Curl_client_write(conn, CLIENTWRITE_BODY,
                  demux.msg.data.datap, demux.msg.data.len);
                if(result) {
                  spindly_free_demux(conn->phys_client, &demux);
                  return result;
                }
                data->req.bytecount += (long)demux.msg.data.len;
              }
          //TODO: should close after stream is closed from both sides
              if(demux.msg.data.flags == 0x1) {
                ...
                stream_client=spindly_stream_get_stream(conn->phys_client,
                  demux.msg.stream.streamid);
                spint = spindly_stream_wndupdate(stream_client,-1);
                if(spint != SPINDLYE_OK)
                  break;
                spint = spindly_phys_outgoing(conn->phys_client,
                  &spdy_packet, &datalen);

                /* send away the window packet */
                //fprintf(stderr,"Sending Data Length = %d\n",datalen);
                //for(i=0;i<datalen;i++)
                //  fprintf(stderr,"[%d] ",spdy_packet[i]);
                ...
                Curl_add_buffer(req_buffer, spdy_packet, datalen);
                result = Curl_add_buffer_send(req_buffer, conn,
                                            &request_size, 0, FIRSTSOCKET);
                if(result == CURLE_OK) {
                  /* tell spindly how much of that data that was sent */
                  spindly_phys_sent(conn->phys_client, datalen);
                }
                else{
      ...
      spindly_free_demux(conn->phys_client, &demux);
    }while(spint == SPINDLYE_DATA_LEFT);
  }
  else {
    /*TODO should define some spdy errors */
    return -1;
  }
/* consumed all data*/
*nread = 0;
...
CURLcode Curl_spdy_setup_connection(struct connectdata *conn)
{
  //fprintf(stderr, "[venki]%s:%d enter\n", __FUNCTION__, __LINE__);
  return CURLE_OK;
}
...
/*
 * Curl_spdy() gets called from the generic Curl_do() function when a HTTP
 * request is to be performed. This creates and sends a properly constructed
 * HTTP request.
 */
CURLcode Curl_spdy(struct connectdata *conn, bool *done)
{
  struct SessionHandle *data=conn->data;
  CURLcode result=CURLE_OK;
  struct HTTP *http;
  ...
  memset(&header, 0, sizeof(struct spindly_headers));
  ...
  *done = TRUE;
  ...
  if((conn->handler->protocol&(CURLPROTO_SPDY)) &&
     data->set.upload) {
    httpreq = HTTPREQ_PUT;
  }
  ...
  if(!Curl_checkheaders(data, "Accept-Encoding:") &&
    ...
    conn->allocptr.accept_encoding =
      aprintf("%s", data->set.str[STRING_ENCODING]);
...
#define TE_HEADER "TE: gzip\r\n"
  ...
  if(ptr && (!data->state.this_is_a_follow ||
    ...
    if(((conn->given->protocol&CURLPROTO_HTTPS) &&
      ...
      conn->allocptr.host = aprintf("%s%s%s",
                                    conn->bits.ipv6_ip?"[":"",
                                    host,
                                    conn->bits.ipv6_ip?"]":"");
    else
      conn->allocptr.host = aprintf("%s%s%s:%hu",
                                    conn->bits.ipv6_ip?"[":"",
                                    host,
                                    conn->bits.ipv6_ip?"]":"",
                                    conn->remote_port);
  ...
  http->p_accept = Curl_checkheaders(data, "Accept:")?NULL:"*/*";
  ...
  /* Use 1.1 unless the user specifically asked for 1.0 or the server only
     supports 1.0 */
  httpstring= use_http_1_1(data, conn)?"HTTP/1.1":"HTTP/1.0";
  ...
  /* GET/HEAD/POST/PUT */
  if(http->p_accept) {
    result = spindly_add_header(&header,"accept", http->p_accept);
    if(result)
      return result;
  }
  ...
  /* url */
  if(conn->allocptr.host) {
    result = spindly_add_header(&header,":host", conn->allocptr.host);
    if(result)
      return result;
  }

  result = spindly_add_header(&header, ":method", request);
  if(result)
    return result;

  result = spindly_add_header(&header,":scheme", "https");
  if(result)
    return result;


  result = spindly_add_header(&header,":path", ppath);
  if(result)
    return result;

  result = spindly_add_header(&header,":version", httpstring);
  if(result)
    return result;

  if(conn->allocptr.accept_encoding)
  result = spindly_add_header(&header,"accept-encoding",
    conn->allocptr.accept_encoding);
  ...
  if(result)
    return result;
  //TODO: need to check cookie support

  if(data->set.timecondition) {
    result = Curl_spdy_add_timecondition(data, &header);
    if(result)
      return result;
  }

  result = Curl_spdy_add_custom_headers(conn, &header);
  if(result)
    return result;
#endif
  ...
  case HTTPREQ_POST_FORM:
    if(!http->sendit || conn->bits.authneg) {
      /* nothing to post! */
      result = spindly_add_header(&header,"Content-Length", "0");
      if(result)
        return result;

      spint = spindly_stream_new(conn->phys_client, 0x1, 0,
        &(data->req.stream_client), NULL, NULL, &header);
      /* get data to send over the socket */
      spint = spindly_phys_outgoing(conn->phys_client, &spdy_packet, &datalen);
      Curl_add_buffer(req_buffer, spdy_packet, datalen);
      result = spindly_phys_sent(conn->phys_client, datalen);
      //fprintf(stderr, "[venki]%s:%d sending\n", __FUNCTION__, __LINE__);
    ...
    if(!data->req.upload_chunky &&
      ...
      snprintf(post_size, 32, "%" FORMAT_OFF_T "\r\n", http->postsize);
      result = spindly_add_header(&header,"Content-Length", post_size);
      if(result)
        return result;
    }

    result = spdyexpect100(data, conn, &header);
    if(result)
      return result;
  ...
  case HTTPREQ_PUT: /* Let's PUT the data to the server! */
    ...
    Curl_pgrsSetUploadSize(data, postsize);
       ...
       spint = spindly_stream_new(conn->phys_client, 0, 0,
         &(data->req.stream_client), NULL, NULL, &header);
      /* get data to send over the socket */
      spint = spindly_phys_outgoing(conn->phys_client, &spdy_packet, &datalen);
      Curl_add_buffer(req_buffer, spdy_packet, datalen);
  ...
  case HTTPREQ_POST:
    ...
    if(!Curl_checkheaders(data, "Content-Type:")) {
      //TODO
      //result = spindly_add_header(&header,"Content-Type",
    ...
    if(ptr) {
      ...
      /*fin flag 0, as we have to send data packet*/
      spint = spindly_stream_new(conn->phys_client, 0, 0,
        &(data->req.stream_client), NULL, NULL, &header);
      /* get data to send over the socket */
      spint = spindly_phys_outgoing(conn->phys_client, &spdy_packet, &datalen);
        ...
        /* if we don't use expect: 100  AND
           ...
           if(!data->req.upload_chunky) {
             /* We're not sending it 'chunked', append it to the request
                ...
                spint = spindly_stream_data(data->req.stream_client, 0x1,
                  data->set.postfields, postsize, NULL);
                /* get data to send over the socket */
                spint = spindly_phys_outgoing(conn->phys_client,
                  &spdy_packet, &datalen);
                Curl_add_buffer(req_buffer, spdy_packet, datalen);
                result = spindly_phys_sent(conn->phys_client, datalen);
                //fprintf(stderr, "[venki]%s:%d\n", __FUNCTION__, __LINE__);
